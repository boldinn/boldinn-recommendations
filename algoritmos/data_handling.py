...
def procesar_datos(org_id, position, knowledge, area):
    ...
    response = requests.get(f"{url}/{org_id}", timeout=60)

    # crear dataframe
    data_df = pd.DataFrame(response.json())

    # cargar datos de desafíos
    desafios_df = pd.read_csv(
        "./algoritmos/desafíos.csv", encoding="ISO-8859-1", delimiter=";"
    )
...
ejercicios_df = pd.read_csv("./algoritmos/ejercicios.csv")
