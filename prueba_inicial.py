...
import pandas as pd
from algoritmos.recomendaciones_inicial import recomendar_ejercicios
import json
...
def obtener_puntajes(group_id):
    criterion_answers = grupos_df.loc[
        grupos_df["group_id"] == group_id, "criterion_answers"
    ].values[0]
    return criterion_answers
...
for g in grupos:
    grupo_id = g
    ejercicios_recomendados = recomendar_ejercicios(
        json.loads(obtener_puntajes(grupo_id))
    )
    print(
        f"""***RUTA DE EJERCICIOS RECOMENDADOS GRUPO {g}***:
