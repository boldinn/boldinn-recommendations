...
import pandas as pd
import numpy as np
from algoritmos.data_handling import ejercicios_df, codigos_criterios
...
class Grupo:
    ...
    def recomendar_ruta_y_actualizar(self):
        ...
        utilidades = [
            1 if n_criterio(c) < 10 else -1 for c, e in recomendaciones.items()
        ]
...
for j, grupo in enumerate(grupos):
    plt.plot(
        range(1, n + 1),
        [promedios[j] for promedios in promedios_por_iteracion],
