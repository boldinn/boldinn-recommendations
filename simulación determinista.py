import random
import json
from recomendaciones import recomendar_ejercicios, ejercicios_df
from alimentar_modelo import actualizar_datos_entrenamiento  
import pandas as pd
import numpy as np
from data_handling import ejercicios_df, codigos_criterios

# Encontrar n (0-19) correspondiente al criterio
def n_criterio(codigo_criterio):
    i=int(np.where(codigos_criterios==codigo_criterio)[0][0])
    return i
...
class Grupo:
    ...
    def recomendar_ruta_y_actualizar(self):
        ...
        try:
            recomendaciones = recomendar_ejercicios(puntajes, self.ejercicios_realizados)
        except:
            self.WIN=True
        ...
        utilidades = [1 if n_criterio(c)<10 else -1 for c,e in recomendaciones.items()]
        
        # c) Simular si los ejercicios fueron aprobados y buscar su valor
        valores_ejercicios = {}
        for criterio, ejercicio in recomendaciones.items():
            aprobado = n_criterio(criterio)<10
            valor_ejercicio = ejercicios_df.loc[ejercicios_df['code'] == ejercicio, 'value'].item()
            valores_ejercicios[ejercicio] = valor_ejercicio if aprobado else -valor_ejercicio
        ...
        for ejercicio, valor in valores_ejercicios.items():
            criterio = ejercicios_df.loc[ejercicios_df['code'] == ejercicio, 'criterion_code'].item()
...
grupos_df['criterion_answers'] = grupos_df['criterion_answers'].apply(lambda x: json.loads(x.replace('=>', ':')))
...
for index, row in grupos_df.iterrows():
    puntajes = row['criterion_answers']
    grupo = Grupo(group_id=row['group_id'], puntajes=puntajes)
    grupos.append(grupo)
...
for i, grupo in enumerate(grupos):
    plt.plot(range(1, n+1), [promedios[i] for promedios in promedios_por_iteracion], label=f'Grupo {grupo.group_id}')
