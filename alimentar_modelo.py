...
def actualizar_datos_entrenamiento(utilidades, puntajes_ejercicios, puntajes):
    ...
    score = 0.6 * promedio_utilidades + 0.4 * suma_puntajes / 48
    ...
    nueva_fila.update({f"C{i + 1}": 0 for i in range(20)})  # Inicializar codificación de criterios en 0

    for codigo in codigos_ejercicios:
        # Encontrar el criterio correspondiente al ejercicio
        criterio = ejercicios_df[ejercicios_df['code'] == codigo]['criterion_code'].values
